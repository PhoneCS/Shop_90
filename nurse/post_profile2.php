...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if ($post_count > 0) {
        ...
        $major = $_POST['major'] ?? ''; // เพิ่มฟิลด์นี้

        // ตรวจสอบข้อมูลที่กรอก
        if (empty($company_name) || empty($job_title) || empty($job_type) || empty($location) || empty($salary) || empty($education_requirements) 
            || empty($company_benefits) || empty($contact_channels) || empty($contact_person) || empty($business_type) || empty($province) || empty($district) 
            || empty($sub_district) || empty($postal_code) || empty($qualifications) || empty($contact_email) || empty($faculty) || empty($major)){
            echo "<p>กรุณากรอกข้อมูลให้ครบถ้วน</p>";
        } else {
             // เตรียมคำสั่ง SQL สำหรับการโพสต์ใหม่
    $stmt = $conn->prepare("INSERT INTO graduates2 (company_name, job_title, job_type, location, salary, education_requirements,
    company_benefits, contact_channels, email, contact_person, business_type, province, district, sub_district, postal_code, qualifications, 
    contact_email, faculty, major) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

   $stmt->bind_param("sssssssssssssssssss", $company_name, $job_title, $job_type, $location, $salary, 
   $education_requirements, $company_benefits, $contact_channels, $email, $contact_person, $business_type,
   $province, $district, $sub_district, $postal_code, $qualifications, $contact_email, $faculty, $major);

   if ($stmt->execute()) {
       // ดึงข้อมูลโพสต์ล่าสุด
       $post_id = $stmt->insert_id; // ได้ ID ของโพสต์ล่าสุดที่เพิ่ม
       $stmt->close();
       
       // ปิดการเชื่อมต่อฐานข้อมูล
       $conn->close();
       
       // รีไดเรกต์ไปยังหน้า index2.php
...
<head>
    ...
    <title>โพสต์งานใหม่</title>
    <link rel="stylesheet" href="styles_post2.css">
</head>
<body>
    <div class="container">
        <h1>โพสต์งานใหม่</h1>
        ...
        <div class="welcome">
            ...
            <div class="links">
                <a href="index2.php">กลับไปที่หน้าโพสต์งาน</a>
        ...
        <?php if ($post_count > 0): ?>
            <p class="alert">คุณได้โพสต์งานไปแล้วหนึ่งครั้ง ไม่สามารถโพสต์เพิ่มได้</p>
        <?php else: ?>
            <form action="post_profile2.php" method="POST">
                <!-- ข้อมูลทั่วไป -->
    ...
    <select id="education_requirements" name="education_requirements">
        <option value="ปริญญาตรี">ปริญญาตรี</option>
                ...
                <div class="form-group">
                    ...
                    <textarea id="company_benefits" name="company_benefits" rows="4" required></textarea>
                </div>
    ...
    <select id="province" name="province" required>
        ...
        ?>
                ...
                <div class="form-group">
                    ...
                    <textarea id="contact_channels" name="contact_channels" rows="4" required></textarea>
                </div>
                <div class="form-group">
